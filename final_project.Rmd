...
# Define the first mean equation
mean_eq1 <- list(armaOrder = c(0, 1, 1),
                  seasonal = list(order = c(2, 0, 0), period = 12), distribution.model = "sstd")
# Define the variance equation
variance_eq <- list(model = "sGARCH", garchOrder = c(1, 1))
# Define the first spec
spec1 <- ugarchspec(mean.model = mean_eq1, variance.model = variance_eq)
# Define the second mean equation
mean_eq2 <- list(armaOrder = c(0, 1, 1),
                 seasonal = list(order = c(1, 0, 2), period = 12),
                 include.mean = TRUE, distribution.model = "sstd")
# Define the second spec
spec2 <- ugarchspec(mean.model = mean_eq2, variance.model = variance_eq)

# Fit the bivariate GARCH model
fit <- ugarchfit(data = data1, multispec = list(spec1, spec2), solver = "hybrid")
...
spec7 <- ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(2,2)),
                   mean.model = list(armaOrder = c(fit_arima_x$arma[1],
                    fit_arima_y$arma[1])), distribution.model = "sstd")
fit7 <- ugarchfit(spec7, data = data1, solver.control = list(trace=0))
fit7 # AIC is 7.6397
aic <- c(aic, 7.6397)
```
